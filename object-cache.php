...
class WP_Object_Cache {  // phpcs:ignore
		...
		try {
			...
			if ( ! $this->should_version( $redis_key ) ) {
				list($value, $found) = self::decode_redis_get(
					$this->call_redis( 'get', array( $redis_key ) ),
				);
				return $value;
			}

			$res = $this->call_redis('mget', array(
				array(
					$redis_key,
					$this->version_key( $key, $group ),
				),
	...
	private function call_mget( $redis_keys, $redis_group_key_indexes ) {
		...
		$res_count = 0;
		$res_array = $this->call_redis( 'mget', array( $redis_params ) );
		if ( false === $res_array ) {
		...
		assert( count( $res_array ) === $res_count );
	...
	public function get_multiple( $keys, $group = 'default', $force = false ) {
		$normal_res = $this->normalized_get_multiple( array( $group => $keys ), $force );
		$group_kv   = array_key_exists( $group, $normal_res ) ? $normal_res[ $group ] : array();
	...
	public function normalized_get_multiple( $groups, $force = false ) {
		$get_from_cache = function ( $key, $group ) {
			...
			return array( $found ? $this->get_internal( $key, $group ) : null, $found );
		};
		...
		foreach ( $groups as $group => $keys ) {
			...
			foreach ( $keys as $key ) {
				...
				$redis_group_key_indexes[] = array( $group, $key );
			}
		}
		...
		if ( $redis_keys ) {
			...
			array_map(
				...
				array_map( array( $this, 'decode_redis_get' ), $redis_result ),
				$redis_group_key_indexes,
			);
		}
	...
	public function delete( $key, $group = 'default', $force = true ) {
		...
		try {
			...
			if ( ! $this->should_version( $redis_key ) ) {
				return self::decode_redis_del( $this->call_redis( 'del', array( $redis_key ) ) ) === 1;
			}

			$this->clear_cache_version( $redis_key );
			$res = $this->call_redis( 'del', array( $redis_key, $this->version_key( $key, $group ) ) );
	...
	public function decr( $key, $offset = 1, $group = 'default' ) {
		...
		$result = $this->call_redis(
			...
			array(
				...
				array( $this->key( $key, $group ), $offset ),
				1,
			)
		);
	 ...
	 * @return mixed
	 */
	public function call_redis( $method, $args = array(), $retry_count = 0 ) {
		global $wpdb;
			...
			try {
				...
				if ( $this->is_retriable_error_message( $e->getMessage() ) && $retry_count < $this->max_retries ) {
					...
					if ( $this->connect_redis() ) {
						return call_user_func_array( array( $this, 'call_redis' ), array( $method, $args, $retry_count + 1 ) );
					}
				}
